 ...
 */
	public function addWorker($pid, $args) {
		return $this->_redis->hSet(self::$workerKey, $pid, serialize($args)) !== false;
	}
 ...
 */
	public function registerSchedulerWorker($pid) {
		return $this->_redis->set(self::$schedulerWorkerKey, $pid);
	}
 ...
 */
	public function isSchedulerWorker($worker) {
		...
		return $pid === $this->_redis->get(self::$schedulerWorkerKey);
	}
 ...
 */
	public function isRunningSchedulerWorker() {
		$pids = $this->_redis->hKeys(self::$workerKey);
		$schedulerPid = $this->_redis->get(self::$schedulerWorkerKey);
 ...
 */
	public function unregisterSchedulerWorker() {
		return $this->_redis->del(self::$schedulerWorkerKey) > 0;
	}
 ...
 */
	public function getWorkers() {
		$workers = $this->_redis->hGetAll(self::$workerKey);
		$temp = array();
 ...
 */
	public function removeWorker($pid) {
		$this->_redis->hDel(self::$workerKey, $pid);
	}
 ...
 */
	public function clearWorkers() {
		$this->_redis->del(self::$workerKey);
		$this->_redis->del(self::$pausedWorkerKey);
	}
 ...
 */
	public function setPausedWorker($workerName, $paused = true) {
		if ($paused) {
			$this->_redis->sadd(self::$pausedWorkerKey, $workerName);
		} else {
			$this->_redis->srem(self::$pausedWorkerKey, $workerName);
		}
	}
 ...
 */
	public function getPausedWorker() {
		return (array)$this->_redis->smembers(self::$pausedWorkerKey);
	}
